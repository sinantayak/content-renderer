...
@Component({
  ...
  template: `
    @switch (data.level) {
      @case (1) { <h1 [id]="slugify(data.text)" [innerHTML]="data.text"></h1> }
      @case (2) { <h2 [id]="slugify(data.text)" [innerHTML]="data.text"></h2> }
      @case (3) { <h3 [id]="slugify(data.text)" [innerHTML]="data.text"></h3> }
      @case (4) { <h4 [id]="slugify(data.text)" [innerHTML]="data.text"></h4> }
    }
  `,
  styles: [`
    :host { display: block; }
